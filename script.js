...
// ✅ FIX: variável estava sendo usada sem existir
        ...
        .onSnapshot(snapshot => {
            ...
            snapshot.forEach(doc => {
                ...
                if (notif.tipo === 'nova_mensagem') {
                    ...
                    li.innerHTML = `
                        <span><strong>${notif.deNome}</strong> quer ser amigo.</span>
            ...
            if (countSino > 0) {
                ...
                badge.innerText = countSino;
            } else {
                badge.style.display = 'none';
                if (lista.children.length === 0) {
                    lista.innerHTML = "<li style='padding:10px; color:#777;'>Nenhuma notificação nova.</li>";
                }
            }
        });
}
...
function toggleNotificacoes() {
    ...
    const abrindo = (modal.style.display !== 'block');
        ...
        db.collection('usuarios').where('email', '==', emailAmigo).get().then(snapshot => {
            if (!snapshot.empty) {
                snapshot.docs[0].ref.update({
                    amigos: firebase.firestore.FieldValue.arrayUnion(usuarioAtual.email)
                });
            }
        });
    ...
    meusAmigos.forEach(email => {
        ...
        ul.innerHTML += `
            <li class="friend-item">
                <div><span style="font-size:16px; font-weight:bold; color:#ddd;">${nome}</span></div>
                <div class="friend-actions">
                    <button class="btn-icon" onclick="abrirChat('${email}', '${nome}')">💬</button>
    ...
    unsubscribeChat = db.collection('chats')
        ...
        .onSnapshot(snapshot => {
            ...
            snapshot.forEach(doc => {
                ...
                d.className = `chat-msg ${msg.email === usuarioAtual.email ? 'msg-me' : 'msg-friend'}`;
                d.innerText = msg.texto;
                b.appendChild(d);
            });
...
function carregarFeed() {
    ...
    db.collection('cadernos').onSnapshot((snapshot) => {
        ...
        tut.className = "friend-book bg-jeans";
        tut.innerHTML = `<div class="book-label">Como Usar?</div>`;
        tut.onclick = abrirTutorial;
        grid.appendChild(tut);

        snapshot.forEach((doc) => {
            const d = doc.data();
            if (!d.donoNome) return;

            mapaNomesAmigos[d.donoEmail] = d.donoNome;

            if (d.donoEmail === usuarioAtual.email || meusAmigos.includes(d.donoEmail)) {
                const div = document.createElement('div');
                div.className = `friend-book ${d.corCapa || 'bg-draft'}`;
                ...
                div.onclick = (e) => {
                    if (!e.target.className.includes('btn-action')) abrirCadernoParaResponder(doc.id, d);
                };
...
function adicionarNovaLinha(txt = "") {
    const l = document.getElementById('lista-criacao');
    ...
    li.innerHTML = `
        <div class="line-container">
            <span class="number-marker">${n < 10 ? '0' + n : n}.</span>
            <input type="text" class="create-input" value="${txt.replace(/"/g, '&quot;')}"
                style="color:${document.getElementById('colorPickerCriacao').value}"
                placeholder="..."
...
function salvarNovoCadernoNoBanco() {
    const inps = document.querySelectorAll('.create-input');
    const p = [];

    inps.forEach(i => {
        if (i.value.trim()) p.push(i.value);
    });
...
function renderizarPerguntasERespostas(caderno, respostas) {
    ...
    caderno.perguntas.forEach((perg, idx) => {
        ...
        li.innerHTML = `
            <div class="line-container">
                <span class="number-marker">${n < 10 ? '0' + n : n}.</span>
                <span class="question-text">${perg}</span>
            </div>`;
        lista.appendChild(li);
        ...
        if (idCadernoAberto !== 'tutorial') {
            ...
            let txt = "";
            let cor = document.getElementById('colorPickerResposta').value;
...
function ajustarAltura(el) {
    ...
    const arredondado = Math.ceil(h / linha) * linha;
    el.style.height = arredondado + 'px';
}
...
function toggleMenu() {
    ...
    // ao abrir sidebar, fecha notificações
...
function fecharMenus() {
    ...
    const drop = document.getElementById('lista-resultados-busca');
    if (drop) drop.style.display = 'none';
}

// ✅ Navegação por classe (mais estável)
